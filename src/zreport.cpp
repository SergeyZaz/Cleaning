...
ZReport::ZReport(QWidget* parent, Qt::WindowFlags flags)//: QDialog(parent, flags)
{
	ui.setupUi(this);
...
QSize ZReport::sizeHint() const
{
	return QSize(1700, 750);
}

void ZReport::expandAll(bool fCheck)
{
	if (fCheck)
		ui.tree->expandAll();
	else
...
double ZReport::getTariffValue(const QDate &date, int id, int num, QString &txt, double& bonus, double& t_val, bool &fSmena)
{
	double v = 0;
...
double ZReport::getSumma(QTreeWidgetItem* pItemRoot, int col)
{
	double s = 0;
...
void ZReport::updateAllSumm(const QList<int>& cols)
{
...
int ZReport::getTextForPayment(int id, int col, QString &text, QVariantList &vList, double& summa, QString& comm)
{
	vList.clear();
	summa = 0;
...
int ZReport::updateComment(int id, const QString & text)
{
...
void ZReport::findFirstSlot(const QString& text)
{
	curFindId = -1;
	findText(text);
}

void ZReport::findNextSlot()
{
	QString text = ui.txtFind->text();
	findText(text);
}

int ZReport::findText(const QString& text)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
ZTreeDataDelegate::ZTreeDataDelegate(ZReport* Editor, QObject* parent)
